...
import type { getAllBookingRequests } from "@/lib/queries";
...
type Booking = Awaited<ReturnType<typeof getAllBookingRequests>>[number];

// Phase 150 — split out of app/admin/bookings/page.tsx (a Server
// Component) so the new Delete action can manage local list state; layout
// unchanged from before this phase.
export default function BookingRequestsTable({ initialBookings }: { initialBookings: Booking[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-[var(--radius)] border border-border bg-card">
      ...
      {bookings.length === 0 ? (
        <p className="p-6 text-muted-fg">No booking requests found.</p>
      ) : (
            ...
            <tbody>
              {bookings.map((b) => (
                <tr key={b.id} className="border-t border-border align-top">
                  ...
                  <td className="px-5 py-3">
                    <DeleteRowButton
                      endpoint="/api/admin/booking-requests/delete"
                      id={b.id}
                      confirmTitle={`Delete booking request from ${b.name}?`}
                      confirmDescription="This will permanently remove this booking request record."
                      onDeleted={onDeleted}
                    />
                  </td>
                </tr>
