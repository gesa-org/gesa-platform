...
export const SIMPLE_PAGE_ENTRIES: { key: string; label: string; hasDescription: boolean; fallback: SimplePageContent }[] = [
  ...
  // longer renders a simple banner page from this key at all — it now
  // renders the former About page's content instead (page_about_hero/
  // page_about_sections, edited via the Content Manager's "Find Support"
  // tab). FIND_YOUR_THERAPIST_CONTENT_FALLBACK below is left defined
  // (harmless unused export) rather than deleted, per the no-delete
  // convention — nothing reads it anymore.
