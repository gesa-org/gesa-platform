...
## Phase 141: fix the shared root cause behind "Edit mode: On is unreliable" + Theme Body Fonts

**Request:** a detailed bug report and spec: "Edit mode: On" doesn't reliably let an admin click text in the live preview — About's Hero heading specifically, but "other texts across pages have similar issues" too — plus a request to add a large list of fonts ("Theme body Fonts," shown in a reference image of a font picker scrolled to its "Ca"–"Co" section) to the Global Theme typography controls. Explicit instruction: "Do not only patch the About Hero heading... identify and fix the shared architecture/root cause."

**Investigation first, per that instruction — two separate, real, confirmed root causes, not one:**

1. **The shared architecture bug (affects nearly every CTA/nav-link field site-wide).** `components/ui-builder/public/EditorPreviewBridge.tsx`'s document-level, **capture-phase** click listener called `event.stopPropagation()` unconditionally whenever a click landed inside an `<a>`/`<button type="submit">`/`<form>` — intended to stop real navigation/submission inside the admin preview iframe. But a capture-phase `stopPropagation()` halts the event's *entire remaining path*, including the bubble phase back up through the actual click target — so `EditableText.tsx`'s own bubble-phase `onClick` (which does the real `select()`/postMessage work) never ran for any `EditableText` nested inside a link/button. Traced through the real DOM and confirmed this affects: every Header nav item and the Donate button, About Hero's two CTA labels, most of Footer's link-wrapped labels (Find a Therapist, Join a Group, Volunteer, Emergency Contact, FAQ, Contact, all 5 legal-page labels), Crisis Button's 4 resource title/description pairs, Home's 3 path-card CTAs, Donate's hero CTA, About's movement/team CTAs, Donate Thank You's back link, and Intake's "more helplines" link — i.e. most of the site's registered CTA/nav-link contentIds, exactly matching "some texts respond, some don't," not a per-page issue.
   - **Fix:** `preventDefault()` alone (still called unconditionally) already blocks the browser's real navigation/submission — it was never necessary to also call `stopPropagation()` to do that job. Now `stopPropagation()` is only called when the click did *not* land on (or inside) a registered `data-gesa-content-id` element, so a plain decorative link still can't fire anything else, while a click on an editable field lets that field's own click handler run normally, exactly as every non-link-wrapped field already did.
2. **About's Hero heading specifically wasn't wrapped in `<EditableText>` at all** (a separate, disclosed gap from Phase 135, not a regression): `components/Hero.tsx` fed `content.title`/`content.highlight` through `<HighlightedText>`, a component that needs plain strings (it does `text.indexOf`/`text.slice()` to split around the highlighted word) — not a React node, so it could never be wrapped in `<EditableText>`. The rendered `<h1>` had no `data-gesa-content-id` and no click handler at all.
   - **Fix:** added `buildHighlightedHtml()` to `components/ui/HighlightedText.tsx` — same highlight-splitting logic as the existing React component, but returning a plain HTML string (with the split segments HTML-escaped) instead of JSX, so `<EditableText html value={buildHighlightedHtml(...)} as="h1">` can render it directly as the real semantic heading. If the field's value already contains HTML tags (meaning an admin has since applied real inline rich-text formatting via this field's toolbar — its type is "heading," which gets the inline rich-text mode), the function skips the highlight-split entirely and returns the value unchanged, rather than risk splitting `text.indexOf`/`slice()` mid-tag and corrupting that formatting — a no-op fallback, never a corruption. The existing `HighlightedText` React component itself is untouched and still used nowhere else (confirmed via a repo-wide search — this was the only call site).

**Escape-to-clear-selection** (spec requirement, found completely missing): added a `keydown` listener for `Escape` in both halves of the postMessage protocol — `EditorPreviewBridge.tsx` (iframe side, since keyboard focus after a canvas click stays inside the iframe's own document, which the parent page's own listeners never see) posts a new `GESA_EDITOR_CLEAR_SELECTION` message up to the parent; `PageEditorShell.tsx` handles that message *and* has its own `Escape` listener for when focus is in the admin chrome instead (e.g. right after clicking a Layers row). Both paths only ever clear `selectedContentId` — never touch `crm_ui_drafts` or any saved field value — so no draft change is ever lost, per the spec's own wording.

**What this does *not* change, and why:** the request also asks about formatting-load-on-select being "inconsistent," draft/publish race conditions, and per-field formatting correctness. All of that machinery (the field registry, `resolveEditorPreview`, the draft/publish API routes, `RichTextEditor`'s value binding to `selectedContentId`) was already keyed correctly off `selectedContentId` from Phase 133 onward — the reported inconsistency traces back to selection itself silently failing for link-wrapped fields (root cause #1 above), not a second, independent bug in the formatting-load path. No changes were made there; fixing #1 is expected to resolve the perceived formatting inconsistency as a direct consequence, since the inspector can only load/show the right field's formatting once it's reliably told which field was actually clicked.

**Theme Body Fonts:** added every font name from the reference image (Calibri through Corbel, ~30 entries including the 5-way Cascadia Code/Mono weight variants) to `BODY_FONT_OPTIONS` in `lib/ui-builder/types.ts` — the single registry `UIBuilderShell.tsx`'s Global Theme "Body font" dropdown reads directly, and that `lib/ui-builder/richTextFontOptions.ts`'s Page Content per-field font dropdown already merges in automatically (confirmed both consume the same exported array, not two separate hardcoded lists — this was already true before this phase). Every one of these ~30 names is a Microsoft/Windows-bundled font (the Office "ClearType Font Collection" — Calibri, Candara, Constantia, Corbel, Cambria — plus classic Windows fonts and the open-source-but-not-on-Google-Fonts Cascadia Code/Mono); none are loadable from Google Fonts and this site doesn't self-host any of them, so each is listed as itself first with a CSS fallback chain to a close, already-available font and finally a generic family — same "degrade gracefully, never break" approach as Phase 137's Aptos and Phase 138's original 5 Office fonts, and consistent with that same phase's confirmed rule against adding fonts nothing can actually render consistently. The 5 that already existed in `richTextFontOptions.ts`'s `EXTRA_FONT_OPTIONS` (Calibri, Cambria, Candara, Constantia, Corbel — added in Phase 138, but never to this Global Theme dropdown before now) reuse their **exact existing value strings verbatim**, so any already-published theme or per-field selection using one of those 5 keeps resolving to the identical CSS stack — nothing legacy silently changes meaning, and the existing `dedupeByValue` merge naturally collapses them to one dropdown row instead of a duplicate. `RICH_TEXT_FONT_FAMILY_OPTIONS`'s `""` ("Default (theme body font)") and `RICH_TEXT_FONT_SIZE_OPTIONS`'s `""` ("Default") already meant "inherit, no inline override" before this phase — confirmed unchanged, no new work needed there. The Global Theme body font is applied via the existing `--font-sans` CSS custom property (`lib/ui-builder/tokensToCss.ts`), shared by both the real site (`app/layout.tsx`) and the admin preview iframe — functionally the same mechanism the spec's own example (`--theme-body-font`) describes, just under the name this codebase already uses everywhere; renaming that variable site-wide was out of scope and not requested.

**Tests:** added `tests/unit/EditorPreviewBridge.test.tsx` — two cases: (1) an `EditableText` nested inside a real `<a>` now correctly posts `GESA_EDITOR_SELECT_ELEMENT` when clicked in edit mode, *and* the click's `defaultPrevented` is still `true` (this test fails against the pre-fix code, since propagation used to get stopped before `EditableText`'s handler ever ran); (2) a plain, non-editable link still can't navigate, edit mode on or off. **Could not actually execute this test in this sandbox** — `npx jest`, even with `--listTests` (which does no test execution at all, only file discovery), hung past this environment's ~180-second command timeout on every attempt. This is a sandbox limitation, not something introduced by this phase — the same "no dev server, no live browser" caveat every UI Builder phase since 132 has disclosed, now confirmed to extend to running the test suite itself here. The test's logic was verified by hand-tracing the exact capture/bubble propagation semantics described above; **running `npm test tests/unit/EditorPreviewBridge.test.tsx` in CI or locally is the most important next verification step**, ahead of any manual click-through.

**QA:** `npx tsc --noEmit` — identical to the established 16-line baseline (the same pre-existing `lib/email/resend.ts` + 5 test-file typing issues), confirmed after every batch of changes this phase. Grepped every new/modified file for the unescaped-apostrophe JSX pattern — every apostrophe found is inside a comment, none in literal JSX text.
**Not verified: an actual click-through in a live browser** (standing sandbox limitation, and per above, the automated test as well). Most important to verify after deploying, in this exact order: (1) open Page Content, select About, turn Edit mode on, click the Hero heading directly — confirm it selects `about.hero.heading` and the inspector loads its current text; (2) click a Header nav label, a Footer link, and a CTA button label on at least 3 different pages — confirm each now selects correctly (previously these silently did nothing); (3) confirm clicking any of those same CTA/nav links in edit mode does **not** navigate away from the builder; (4) press Escape after selecting something — confirm the Layers panel/inspector deselect and no draft change is lost; (5) open the Global Theme tab's "Body font" dropdown — confirm all ~30 new fonts appear (Calibri through Corbel) and selecting one applies immediately in the preview; (6) open a Page Content field's font-family dropdown — confirm the same 5 pre-existing fonts (Calibri/Cambria/Candara/Constantia/Corbel) still appear exactly once, not twice.

**Files changed:** `components/ui-builder/public/EditorPreviewBridge.tsx` (capture-phase click fix, Escape handling), `components/admin/ui-builder/PageEditorShell.tsx` (Escape handling, `GESA_EDITOR_CLEAR_SELECTION` message), `components/ui/HighlightedText.tsx` (`buildHighlightedHtml` added, existing component untouched), `components/Hero.tsx` (Hero heading now rendered through `EditableText`), `lib/ui-builder/types.ts` (`BODY_FONT_OPTIONS` expanded), `tests/unit/EditorPreviewBridge.test.tsx` (new). No new database migrations, no API route changes, no changes to the draft/publish data flow itself.

```
del .git\index.lock
git add -A
git commit -m "Phase 141: fix Edit Mode click-to-select root cause, add Escape-to-clear, add Theme Body Fonts"
git push
```
...
## Phase 142: rebuild the "Find Support" flow (choice screen, AI/Manual pathways, unified CRM)

**Request:** an 11-section spec asking to (1) audit every "Find Support" link site-wide to point at the real Find Support entry point instead of About, (2) replace the direct-to-therapist-list CTA with an "AI Support" / "Manual Support" choice screen, (3) Manual Support redirects to Our Professionals with `?source=manual-support`, (4) AI Support opens a multi-step form collecting session preferences, personal info + consent + age confirmation, and an open-text "how are you feeling" field with a crisis disclaimer, (5) extend the matching engine, (6) show 3–5 result cards, (7) wire therapist selection into scheduling with CRM + notification updates, (8) a unified CRM dashboard, (9) minimal-info therapist notifications, (10) a unified `support_requests` data model with RBAC/security, (11) a fully working, tested flow — while explicitly preserving the existing Our Professionals page, visual identity, and every other existing page.

**Architectural decisions confirmed with Roy up front (three-question check before building), given substantial pre-existing overlapping infrastructure found during investigation** (a full 4-step `MatchWizard`/`/api/match`/`/api/match-booking` flow already live on `/find-your-therapist`, plus a completely separate diary-link/native booking system — `BookSessionButton`, `booking_intake_forms`, `diary_scheduling_events`, `session_bookings` — already built for the Our Professionals directory in earlier phases):
1. **Consolidate into `/find-your-therapist`** rather than a new route — the choice screen renders there first, "AI Support" reveals the existing wizard (extended), "Manual Support" redirects to `/therapists?source=manual-support`.
2. **Unify into one new `support_requests` table**, replacing `match_requests` as the wizard's write target (old table and its admin page left untouched for historical rows — nothing deleted).
3. **Extend the existing AI+rules matcher** (`lib/ai/matchTherapists.ts`) rather than building a second one.

**A production database discovery, surfaced to Roy before any schema work:** this sandbox's `.env.local` points at a Supabase project (`ggjvpfivyqartvanvhzq`, "gesa-dev") that is **missing** `diary_scheduling_events`, `booking_intake_forms`, and `crm_ui_drafts` — three tables live API routes already depend on (the diary-link booking flow, the booking-intake form, and UI Builder autosave). A second project (`iddeoavrlnvwwfopsacy`) has all of those tables plus the full legacy `gesa_*` data. Asked Roy directly rather than guessing or building against the wrong database; confirmed `iddeoavrlnvwwfopsacy` is production. All schema work this phase (below) was applied there, via direct Supabase MCP access, not through a checked-in migration file (this repo has no migration file for anything since the original `0001_init.sql` — every later schema change in this project's history appears to have been applied the same way). **This sandbox's own `.env.local` was not corrected** — it's a separate concern from this phase's scope, and Roy may want to check whether it's genuinely stale locally or whether it's actually a real second (dev/staging) project that's simply behind on migrations; flagging it here rather than silently leaving it unmentioned.

**New table: `support_requests`** (migration `create_support_requests`, applied directly to the `iddeoavrlnvwwfopsacy` project) — one row per "Find Support" journey, both pathways. Columns: `pathway` (`ai`/`manual`), a `status` pipeline (`started` → `preferences_submitted` → `matched`/`no_match` → `therapist_selected` → `booking_requested`/`scheduled` → `completed`/`cancelled`, plus `redirected_manual` for Manual Support rows), personal info (`full_name`/`email`/`phone`/`age_confirmed`/`consent_at`), preferences (`gender_preference`/`support_categories`/`treatment_type`/`preferred_language`/`session_format`/`clinic_location_id`/`availability_notes`/`accessibility_needs`), the open-text `feelings_text` + `crisis_disclaimer_shown_at`, matching results (`matched_therapist_ids`/`ai_reasoning`/`gender_preference_honored`), and scheduling handoff (`selected_therapist_id`/`preferred_date`/`preferred_time`/`diary_scheduling_event_id`, the last a real FK into the existing `diary_scheduling_events` table so scheduling state is never duplicated between the two). **RLS enabled with zero policies** — matching `booking_intake_forms`' existing posture, deliberately stricter than `match_requests` because this table collects more sensitive data (the open-text feelings field) — every read/write goes through a server-only route using the service-role admin client, confirmed via `get_advisors` (only the expected "RLS enabled, no policy" info-level finding, nothing else new). Also hardened the `updated_at` trigger function with `search_path = ''` after `get_advisors` flagged it as mutable.

**Flow, end to end:**
- `components/find-support/ChoiceScreen.tsx` + `FindSupportFlow.tsx` — the new first thing a client sees on `/find-your-therapist`. "Manual Support" (and the screen's own close/X button, which skips the question entirely) logs the pathway via `/api/support-pathway` then redirects to `/therapists?source=manual-support`; "AI Support" reveals the wizard.
- `MatchWizard.tsx` now creates its own `support_requests` row on mount (pathway `ai`) and grew from 4 steps to 6: **Support Needs** (existing symptom picker, unchanged) → **Preferences** (existing gender/treatment fields, now also preferred language, availability, accessibility needs) → **Format & Location** (unchanged) → **Your Info** (new — full name/email/phone, explicit 18+ confirmation, consent checkbox, validated on Continue) → **Feelings** (new — open-text field + the required crisis disclaimer and a direct 988 link) → **Matches**.
- `/api/support-match` (new, replacing `/api/match` for this flow — that route is untouched, just no longer called from here) saves every collected answer onto the `support_requests` row for the first time, then runs the same roster/matching logic as before.
- `lib/ai/matchTherapists.ts` extended: `preferredLanguage` and `feelingsText` are both soft signals only (never hard filters, unlike gender) — the AI prompt is told to weight language like treatment type and use the feelings text purely as fit-judging context, explicitly never to quote it back in `reasoning`; the rule-based fallback adds a small language-match score bonus.
- **Results + booking now reuse the existing, already-robust `BookSessionButton`** (diary-link vs native branching, full self-report slot-selection flow, already used on the Our Professionals directory) instead of the old flow's own narrower booking modal — `StepMatches.tsx` is now presentational (matches/loading/error as props) and renders `<BookSessionButton therapist={t} pathKey="ai-support" supportRequestId={...} onFirstInteract={...} />` per match card. `BookSessionButton` gained two small optional props to support this: `onFirstInteract` (fires once, the moment either booking path opens — used to log therapist selection) and `supportRequestId` (passed through to `/api/diary-scheduling`, which now links the resulting event back to the `support_requests` row and advances its status to `scheduled`). `TherapistMatch.therapist` widened from a narrow field `Pick` to the full `PublicTherapistRow` so it carries everything `BookSessionButton` needs (slug, diary_link, etc.).
- `/api/support-request/select-therapist` (new) — called on that same first interaction; records `selected_therapist_id`/`status: therapist_selected` and sends two minimal-info emails (therapist + team) via two new templates in `lib/email/templates.ts` — deliberately never including `feelings_text`, matching the spec's "no full feelings text in unsecured email" requirement.
- **Scope boundary, disclosed rather than silently left incomplete:** the *diary-link* booking path is fully cross-linked back to `support_requests` (via `diary_scheduling_event_id`, above). The *native* (non-diary) booking path — `BookSessionButton`'s other branch, going through `IntakeBookingModal`/`/api/intake-booking` into `session_bookings` — was **not** threaded with a similar link in this phase; a `support_requests` row for a native booking will show `therapist_selected` as its final status from this flow, with the actual reservation visible in `session_bookings` (matchable by therapist + client email) but not automatically cross-referenced by id in the CRM view. Deepening that would mean touching `IntakeBookingModal`/`BookingIntakeModal`/`/api/intake-booking`'s existing, already-in-production request bodies — judged out of scope for this phase given the size of everything else here; flagging as a natural next increment rather than leaving it unstated.

**Unified CRM dashboard:** `app/admin/support-requests/page.tsx` (new) — one table over every `support_requests` row, both pathways, with a pathway badge, client info, format, selected professional, a preferences column (including a collapsible "How they're feeling" detail), and a status dropdown. Reads via `getAllSupportRequests()` in `lib/queries.ts`, which deliberately uses the service-role admin client rather than this file's usual cookie-based pattern (every other function in that file relies on an RLS policy scoped to the signed-in admin; this table has none) — safe specifically because this function is only ever called from this one page, itself behind `requireAdmin()` in `app/admin/layout.tsx`. The status dropdown (`SupportRequestStatusSelect.tsx`) can't write straight through the browser Supabase client the way `MatchRequestStatusSelect` does for the same reason, so it posts to a new `/api/admin/support-requests/status` route that re-checks the caller's own session via `getCurrentProfile()` (not `requireAdmin()`, which is written to `redirect()` and is wrong for an API route) before writing. The old `/admin/match-requests` page and its nav entry are untouched, relabeled "(legacy)" — still there for whatever rows exist in `match_requests` from before this phase, receiving no new writes going forward.

**Therapist-facing notifications:** the existing `app/therapist/page.tsx` dashboard gained a third section, "AI Support Bookings," listing every `support_requests` row where `selected_therapist_id` matches the signed-in therapist (via the same admin-client + explicit `.eq(therapist_id, ...)` defense-in-depth pattern the page's other two queries already use, necessary here since there's no RLS policy to also rely on). Combined with the minimal-info selection email above, this satisfies the "AI Support Bookings" dashboard + notification requirements without duplicating the existing session-booking/diary-handoff sections already on that page.

**QA:** `npx tsc --noEmit` — identical to the established 16-line baseline after fixing one real error this phase introduced and caught (a `supportRequestId` prop referenced before it was destructured in `BookSessionButton.tsx`). Grepped every new/modified file for the unescaped-apostrophe JSX pattern — every match is inside a comment or a `.ts`-file string, none in literal JSX text. **Not verified: an actual click-through in a live browser or the automated test suite** — same standing sandbox limitations disclosed every phase since 132 (`npx jest`, even `--listTests`, hangs past this environment's command timeout; no dev server here). Most important to verify after deploying, in order: (1) every site-wide "Find Support" link/button lands on the choice screen, not About; (2) Manual Support redirects to `/therapists?source=manual-support`; (3) the full AI Support wizard end to end, confirming a `support_requests` row appears in the new admin CRM page at each step; (4) a diary-link therapist's booking flow from the results screen actually advances that row to `scheduled`; (5) the therapist notification email arrives and does not contain the feelings text; (6) the new "AI Support Bookings" section appears on that therapist's own dashboard.

**Files changed:** `lib/database.types.ts` (`SupportRequestRow`/`SupportPathway`/`SupportRequestStatus` types + `Tables` entry), `lib/queries.ts` (`getAllSupportRequests`), `lib/ai/matchTherapists.ts` (language + feelings-text signals), `lib/email/templates.ts` (2 new templates), `components/find-support/ChoiceScreen.tsx` + `FindSupportFlow.tsx` (new), `components/match/MatchWizard.tsx` (rewritten — 6 steps, own support_requests row), `components/match/StepPreferences.tsx` (language/availability/accessibility fields added), `components/match/StepPersonalInfo.tsx` + `StepFeelings.tsx` (new), `components/match/StepMatches.tsx` (rewritten — presentational, uses `BookSessionButton`), `components/match/types.ts` + `constants.ts` (extended), `components/therapists/BookSessionButton.tsx` (`onFirstInteract`/`pathKey`/`supportRequestId` props added), `components/admin/SupportRequestStatusSelect.tsx` (new), `app/find-your-therapist/page.tsx` (renders `FindSupportFlow`), `app/admin/support-requests/page.tsx` (new), `app/admin/layout.tsx` (new nav entry), `app/therapist/page.tsx` (AI Support Bookings section), `app/api/support-pathway/route.ts` + `app/api/support-match/route.ts` + `app/api/support-request/select-therapist/route.ts` + `app/api/admin/support-requests/status/route.ts` (all new), `app/api/diary-scheduling/route.ts` (optional `supportRequestId` linking). `components/match/BookingModal.tsx` is now unused (superseded by `BookSessionButton` reuse) — left in place rather than deleted; a candidate for cleanup in a future phase. New Supabase migrations on the `iddeoavrlnvwwfopsacy` project: `create_support_requests`, `harden_support_requests_trigger_search_path`.

```
del .git\index.lock
git add -A
git commit -m "Phase 142: rebuild Find Support flow — AI/Manual choice screen, unified support_requests CRM, extended matching + booking + notifications"
git push
```

Roy — same as every phase: please run those four one at a time, and paste back what appears directly after the `git commit` line specifically. One more thing worth your attention outside the git block: I found your `.env.local` in this sandbox pointing at a Supabase project that's missing a few tables live features already depend on — worth a quick look at whether that's a real second project lagging behind, or just a stale local config; see the note above for details.

---
**Gate:** Per Roy's instruction, each phase stops here for review/approval before the next one starts.

## Phase 143: shorten the Find Support wizard from 6 steps to 4

**Request:** "the current multi-step process feels confusing because several questions repeat similar information" — remove the "Support Needs" and "Your Info" steps entirely (fields, validation, saved state, progress-indicator references, all of it), remove the Preferences step's Availability and Accessibility fields (from the UI, validation, saved state, backend payloads/DB records, matching logic, and admin views), reorder the remaining steps to Preferences → Format & Location → Feelings → Matches, and — if name/email/consent/phone are actually needed to request a match — move them into a compact Contact Details section on the Matches step itself rather than a separate step. Preserve the "guided, confidential match, no account required" messaging and useful preference/location data for matching.

**What came out, and why each was safe to remove:**
- **"Support Needs"** (`StepAssessment.tsx`, a symptom-checkbox picker) — its `symptoms`/`support_categories` data was passed to the AI matcher as one signal among several (treatment type, gender, language, and — since Phase 142 — the open-text Feelings step). The Feelings step's free text already covers the same ground in the client's own words, so removing the checkboxes drops a redundant, duplicate question rather than a matching-relevant one.
- **"Your Info"** (`StepPersonalInfo.tsx`, name/email/phone/18+ confirmation/consent) — removed as its own step. Name/email/consent/optional phone are genuinely needed (to notify a therapist and log the CRM row when a client requests an introduction), so those four moved into a new "Your contact details" section on the Matches step, gating each match card's booking button until filled in (client-side validation, same as the old step had). **The 18+ age confirmation checkbox was not preserved anywhere** — Roy's instruction listed exactly "name, email, consent, and optional phone number" as what to keep, and said to remove "all fields... associated with" the Your Info step otherwise; age confirmation wasn't on the keep list, so it's gone along with the step. Flagging this explicitly rather than silently dropping a consent-adjacent control: if that 18+ gate was intentional (this is a mental-health-adjacent flow), it's worth confirming before this ships, and it would need to come back as an explicit ask if so.
- **Availability and Accessibility** (two optional free-text fields on the Preferences step, added just one phase ago in Phase 142) — confirmed via the matching engine (`lib/ai/matchTherapists.ts`) that neither ever fed into scoring or filtering; they were stored on the `support_requests` row and nothing else. Removed from the step's UI, its props, the wizard's saved state, the `/api/support-match` payload/DB write, and the admin CRM table (which was never showing them anyway, so no change needed there).

**New 4-step flow:** `MatchWizard.tsx`'s `STEP_LABELS` is now `["Preferences", "Format & Location", "Feelings", "Matches"]`. Preferences (now step 1, first in the flow) lost its Back button entirely — there's nothing before it to go back to, so no dead/no-op button sits there. Every other step's Back/Continue target was renumbered accordingly (Format & Location back→Preferences, Feelings back→Format & Location, Matches back→Feelings). `submitForMatches()` now transitions to `step 3` (was `step 5`) on both success and failure paths.

**Contact Details on the Matches step:** `StepMatches.tsx` gained a "Your contact details" card (full name, email, optional phone, a consent checkbox linking GESA's Privacy Policy) rendered directly above the match cards, only once real matches exist. Each match card's booking action is gated: until name + a valid email + the consent checkbox are all satisfied, the card shows a disabled-looking "Add contact details above" button instead of the real `<BookSessionButton>`; clicking it just reveals the inline validation messages (`touched` state), consistent with this wizard's existing validate-on-attempt pattern from the step it replaced. Once contact details are valid, the real `BookSessionButton` renders and works exactly as it did in Phase 142.

**Backend changes:** `/api/support-match` no longer reads or saves `symptoms`/`availabilityNotes`/`accessibilityNeeds`/`fullName`/`email`/`phone`/`ageConfirmed`/`agreedConsent` — those either don't exist anymore or, for contact details, aren't known yet at that point in the new flow. `/api/support-request/select-therapist` now reads and saves `fullName`/`email`/`phone`/`agreedConsent` (the first and only point where they're known), alongside recording the therapist selection — previously it only read back values `/api/support-match` had already saved, which no longer happens. `lib/ai/matchTherapists.ts`'s `MatchInput.symptoms` was widened from required to optional (rather than removed outright) so the older, still-present-but-unused `/api/match` route — which still sends it — keeps compiling and working unchanged; `/api/support-match` simply omits it now. No new database migration: `support_categories`/`availability_notes`/`accessibility_needs`/`age_confirmed` remain as unused columns on `support_requests` going forward (historical rows before this phase keep whatever they have; new rows will just have empty/default values in those columns) — dropping columns felt like unnecessary risk for a UI-only simplification request, and this was judged the safer call over a destructive schema change.

**Orphaned files, not deleted:** `StepAssessment.tsx` and `BookingModal.tsx` (already unused since Phase 142) both referenced now-removed types/exports (`SYMPTOMS`, `answers.symptoms`) and were breaking `tsc --noEmit` as a result, even though neither is imported anywhere. Per this project's convention against deleting already-written files without asking first, both got a `@ts-nocheck` plus a comment explaining why, restoring a clean typecheck without touching their (dead) runtime behavior. `StepPersonalInfo.tsx` (also now unused) needed no such fix — its own prop types were self-contained and never referenced the changed `WizardAnswers` shape.

**QA:** `npx tsc --noEmit` — back to the established 16-line baseline after adding the two `@ts-nocheck` headers above (confirmed those were the only new errors this phase introduced, both in dead/unused files). Grepped every changed file for the unescaped-apostrophe JSX pattern — every match is inside a comment. **Not verified: an actual click-through in a live browser or the automated test suite** — same standing sandbox limitations disclosed every phase since 132. Most important to verify after deploying: (1) the stepper shows exactly 4 steps, no blank/skipped ones; (2) Preferences (step 1) has no Back button and nothing broken from missing one; (3) the Contact Details section blocks booking until valid, then unblocks correctly; (4) a `support_requests` row shows contact info only appearing at the therapist-selection point, not earlier; (5) the admin CRM table's Preferences column no longer shows a stray empty line where support categories used to render.

**Files changed:** `components/match/types.ts` (`WizardAnswers` trimmed), `components/match/constants.ts` (`SYMPTOMS` removed), `components/match/StepPreferences.tsx` (Availability/Accessibility fields + Back button removed), `components/match/MatchWizard.tsx` (rewritten — 4 steps, StepAssessment/StepPersonalInfo no longer imported, contact-detail props threaded into StepMatches, selection payload now carries contact details), `components/match/StepMatches.tsx` (Contact Details section + gating added), `app/api/support-match/route.ts` (payload/save trimmed), `app/api/support-request/select-therapist/route.ts` (now saves contact details), `lib/ai/matchTherapists.ts` (`symptoms` made optional), `app/admin/support-requests/page.tsx` (support-categories display line removed), `components/match/StepAssessment.tsx` + `BookingModal.tsx` (`@ts-nocheck` added, otherwise untouched — both already unused since Phase 142). No new database migrations.

```
del .git\index.lock
git add -A
git commit -m "Phase 143: shorten Find Support wizard to 4 steps, move contact details to Matches step"
git push
```

Roy — same as every phase: please run those four one at a time, and paste back what appears directly after the `git commit` line specifically. One thing to double-check before this goes live: the old 18+ age confirmation checkbox is gone with the "Your Info" step and wasn't on your list of fields to preserve — if that gate was intentional, let me know and I'll add it back (most naturally as one more line in the new Contact Details section).
...
## Phase 145: replace the standalone Find Support page with the existing About Us page

**Request:** A full reversal of Phase 144's approach — Roy asked to remove the old standalone Find Support page (the "A guided match, just for you" AI/Manual Support choice screen) entirely and put the existing About Us page's content at `/find-your-therapist` instead, preserving every existing About section (hero, "How GESA Works," founder spotlight, movement CTA, Team & Advisors, DonateBand) exactly as is. `/about` needed to become a permanent redirect to `/find-your-therapist`, with no duplicate "Find Support"/"About Us" nav entries anywhere, and the former Find Support CTA on the hero needed new, non-self-referential behavior once it renders on the same page it used to link to.

**What changed:**
- `app/find-your-therapist/page.tsx` — fully rewritten to render what used to be `app/about/page.tsx`'s JSX (Hero + "How GESA Works" + Founder spotlight + Movement CTA band + Team & Advisors + `DonateBand`), reading the same two `site_content` keys (`page_about_hero`, `page_about_sections`) and resolving editor-preview mode under the same `"about"` pageKey the Content Manager already used. `metadata.title` is now "Find Support — GESA". Added `id="how-it-works"` to the "How GESA Works" section as the new scroll target for the hero CTA.
- `app/about/page.tsx` — left in place, unreachable (per the standing no-delete-files rule), with a comment explaining `next.config.mjs`'s redirect means this component never runs in production anymore.
- `next.config.mjs` — added an `async redirects()` sending `/about` → `/find-your-therapist` with `permanent: true` (308), so old links/bookmarks still resolve instead of 404ing.
- `components/Hero.tsx` — `HERO_CONTENT_FALLBACK.ctaPrimaryLabel`/`ctaPrimaryHref` changed from "Find Support" / `/find-your-therapist` to "See How It Works" / `#how-it-works`, since this Hero now renders ON `/find-your-therapist` itself — the old label/href were self-referential once moved. Also updated the matching **live** production `site_content` row for `page_about_hero` directly via Supabase MCP (project `iddeoavrlnvwwfopsacy`) — confirmed via `execute_sql` beforehand that the live row still had the stale `"/find-your-therapist"` / `"FIND SUPPORT"` values, so the code fallback alone wouldn't have changed what's actually published. `scroll-behavior: smooth` is already global (`app/globals.css`), so the new anchor CTA animates with no extra JS.
- `lib/navigation.ts` — removed the `aboutPage` entry (`/about`) from `PRIMARY_NAVIGATION` entirely. Confirmed via a live query that `site_header`'s published row has no `aboutPageLabel` key at all, so nothing needed cleaning up there.
- `lib/ui-builder/pageRegistry.ts` — the `"about"` `PAGE_DEFINITIONS` entry's `route` changed from `/about` to `/find-your-therapist` (title changed to "Find Support"); the separate `"find-your-therapist"` entry (the old simple eyebrow/title/description banner) removed entirely, along with `FIND_YOUR_THERAPIST_EDITABLE_FIELDS` and its `FIELDS_BY_PAGE` lookup row; the orphaned `global.header.aboutPageLabel` field removed from `GLOBAL_EDITABLE_FIELDS`.
- `lib/content.ts` — removed the `page_find_your_therapist` row from `SIMPLE_PAGE_ENTRIES` (that banner content is no longer rendered anywhere). `FIND_YOUR_THERAPIST_CONTENT_FALLBACK` is left defined but now unused, per the no-delete convention.
- `components/SiteFooterSlot.tsx` — `REVEAL_ROUTES` swapped `"/about"` for `"/find-your-therapist"`, matching where the reveal-effect-wrapped content actually lives now.
- `components/admin/content/ContentManagerApp.tsx` — the "About Us" tab renamed "Find Support" (same two editors underneath, `HeroEditor`/`AboutSectionsEditor`, unchanged). The old separate "Find a Therapist" generic-banner tab disappeared automatically once its `SIMPLE_PAGE_ENTRIES` row was removed — no code change needed there.
- `components/admin/content/HeaderEditor.tsx` — removed the `aboutPageLabel` input field and updated the note text to describe the new single-nav-item reality.
- `tests/e2e/navigation.spec.ts` — rewrote the nav-links test: the "Find Support" link now asserts the About-derived heading ("Emotional support should feel human…") instead of the old wizard's; the old "About Us" link-click assertion replaced with a check that no such nav link exists at all; added a new test confirming `/about` redirects to `/find-your-therapist` with the same heading visible.

**What was deliberately left untouched:** the old `FindSupportFlow`/`MatchWizard`/wizard-step component tree (Phase 142/143's work) stays on disk, just no longer imported from any page — same "don't delete already-written code" precedent used for `StepAssessment.tsx`/`BookingModal.tsx` in Phase 143. Footer's "Find a Therapist" link was already hardcoded to `/find-your-therapist` (confirmed via grep) — no change needed there. The open question flagged back in Phase 143 (whether to keep an 18+ age-confirmation checkbox) remains unresolved and wasn't reintroduced by this phase either.

**QA:** `npx tsc --noEmit` — unchanged from the established 16-line baseline (1 `resend.ts` + 15 across the same 5 pre-existing test files). Grepped every changed file for the unescaped-apostrophe JSX pattern — every match is inside a comment; no new literal JSX text with an apostrophe was introduced. Not verified in a live browser (same standing sandbox limitation as every phase since 132) — most important to check after deploying: (1) `/find-your-therapist` shows the full About content with a working "See How It Works" CTA that scrolls to the how-it-works cards; (2) `/about` redirects cleanly (308) with no flash of the old content; (3) the header/footer/mobile show exactly one "Find Support" link and nothing labeled "About Us" anywhere; (4) the Content Manager's "Find Support" tab still edits the live hero/sections content correctly.

**Files changed:** `app/find-your-therapist/page.tsx`, `app/about/page.tsx` (comment only), `next.config.mjs`, `components/Hero.tsx`, `lib/navigation.ts`, `lib/ui-builder/pageRegistry.ts`, `lib/content.ts`, `components/SiteFooterSlot.tsx`, `components/admin/content/ContentManagerApp.tsx`, `components/admin/content/HeaderEditor.tsx`, `tests/e2e/navigation.spec.ts`. Database: one live `site_content` row updated (`page_about_hero`'s `ctaPrimaryHref`/`ctaPrimaryLabel`) via Supabase MCP against production project `iddeoavrlnvwwfopsacy`.

```
del .git\index.lock
git add -A
git commit -m "Phase 145: replace standalone Find Support page with About Us content"
git push
```
...
## Phase 151: "Browse therapist" guided search on the Find Support page

**Request:** Full structured spec (paraphrased): make the Find Support page's "Browse therapist" option open a guided search — session type (online/in-person) + location (country, city/address) — show matching therapists with a result count, let the client pick one and book through the existing "Choose a date and time" flow, and tag the resulting booking internally as `bookingSource: "browse_therapist"` plus the search's session type/country/city. Polished, responsive, accessible, no duplicate therapist data or a second booking flow.

**Discovery, before building anything:** audited the existing Find Support flow (`ChoiceScreen` → `FindSupportModal` → `FindSupportFlow`), the therapist data model (`therapists`/`therapists_public`), and the full booking path (`BookSessionButton` → `IntakeBookingModal`/diary-link flow → `/api/intake-booking` and `/api/diary-scheduling`). Found that "Browse therapist" already existed as a choice-screen option, but it just redirected straight to `/therapists?source=manual-support` — no search step of any kind. More importantly, found that **none of the data the spec asks to filter on actually existed**:

- No column anywhere distinguished a therapist as online-capable vs. in-person-capable — `session_format` is a client-side preference used only by the AI Support wizard's own `support_requests` row, never a therapist attribute, and it isn't even used to filter AI Support's own matches today.
- Therapists have a single free-text `country` column and nothing finer — no `city`, no coordinates. Clinic locations (`clinic_locations`) are a small, shared list of physical GESA offices, not per-therapist addresses.
- The native (non-diary-link) booking flow's only "how should we connect" options were `email`/`whatsapp`/`zoom` — every one of them inherently remote. There was no in-person option in the booking flow at all.
- Neither `session_bookings` nor `diary_scheduling_events` had a source/path field that could hold something like `bookingSource` for the diary-link branch (`session_bookings` already had a `path` column serving exactly that purpose for the native branch; `diary_scheduling_events` had nothing at all).

Building the feature as specified required extending the schema first — done via 4 applied migrations (see below) — rather than inventing a parallel, fake data source. Every default was chosen so no existing record silently claims a capability it never had.

**Database changes (all applied to production project `iddeoavrlnvwwfopsacy`):**

1. `add_browse_search_columns_to_therapists` — added `offers_online` (boolean, default `true`), `offers_in_person` (boolean, default `false`), `city` (text), `latitude`/`longitude` (numeric) to `public.therapists`. Defaults chosen deliberately: every existing therapist is treated as online-capable (the pre-existing flow was already remote-first) and NOT in-person-capable (nothing before this phase distinguished that at all) until an admin says otherwise.
2. `extend_therapists_public_view_for_browse_search` — recreated `public.therapists_public` (the RLS-safe view every public page reads from) to expose `offers_online`/`offers_in_person`/`city`. `latitude`/`longitude` deliberately excluded from the public view — no browser-side code needs raw coordinates.
3. `add_browse_search_metadata_to_bookings` — added `search_session_type`/`search_country`/`search_city_or_address` to both `session_bookings` and `diary_scheduling_events`; added a `path` column to `diary_scheduling_events` for the first time (mirroring `session_bookings.path`'s existing purpose, since that table never had a source tag at all).
4. `add_in_person_contact_channel` — widened `session_bookings`' `contact_channel` CHECK constraint from `email`/`whatsapp`/`zoom` to also allow `in_person`.

Corresponding TypeScript types (`TherapistRow`, `PublicTherapistRow`, `ContactChannel`, `SessionBookingRow`, `DiarySchedulingEventRow` in `lib/database.types.ts`) updated to match.

**Design decision — reused `path`, didn't duplicate it:** `session_bookings.path` already exists and already serves exactly the "which flow did this booking come from" purpose the spec's `bookingSource` field describes (existing values: `directory`, `ai-support`, etc.). Rather than add a second, redundant column, Browse Therapist bookings set `path = "browse_therapist"` — the admin display change (see below) reads from that same field. `diary_scheduling_events` never had this concept at all, so it genuinely needed a new `path` column (added in migration 3 above), added with the same name for consistency.

**New country list, no new dependency:** `lib/countries.ts` builds the Country field's option list from `libphonenumber-js`'s `getCountries()` + the browser's built-in `Intl.DisplayNames` — the exact same technique `components/ui/PhoneNumberInput.tsx` already uses for its own country picker. No new package, no external geocoding/autocomplete service.

**New pure search/validation logic:** `lib/browseTherapistSearch.ts` — `validateBrowseSearch()`/`isBrowseSearchValid()` (the spec's exact 3 validation messages) and `searchTherapists()`, kept out of the React component entirely so it's independently unit-tested. Match rules implemented, given the schema realities above:
- **Online:** therapist must have `offers_online = true`; country matches if the therapist's own country matches the search, OR the therapist has no country on file at all (treated as globally available online).
- **In-person:** therapist must have `offers_in_person = true` AND a country match (in-person is never "global" — meeting in person requires being in the same country). City/address is a soft filter: only excludes a therapist when both the search and the record have a city and neither contains the other; a therapist with no city on file is never excluded on that basis alone.
- **Distance ranking:** wired up (`haversineDistanceKm()`, a `distanceKm` field on every result) but always `null` today, since no therapist record has real coordinates yet — no geocoding step exists anywhere in this app. The UI never displays a distance number, per the spec's own "clearly avoid claiming precise proximity" instruction.

**New component — `components/find-support/BrowseTherapistModal.tsx`:** the guided search itself, in two internal steps (search, then results) inside one modal, opened from `ChoiceScreen`'s "Browse therapist" card. Search step: session type (Online/In-person selectable cards, each with the spec's exact helper text), Country (text input + `<datalist>` autocomplete sourced from `lib/countries.ts` — no existing autocomplete/geocoding service to plug into, so this is the closest honest equivalent), City/Address (required only for in-person). "Start browsing therapists" stays disabled until the required fields for the chosen session type are filled in. Results step: "Therapists available in {country}" or "Therapists near {city}, {country}" heading, live result count, session-type/location filter chips, "Change search" (returns to the search step with every selection intact), and either a results grid or the no-results panel (exact copy from the spec, with "Change search" and "Browse all professionals" — the latter navigates to `/therapists` with `sessionType`/`country`/`city` as query params; **disclosed limitation:** the existing `/therapists` directory (`TherapistsDirectory.tsx`) has no country or session-type filter UI at all today, so those params are attached for reference/future use but aren't yet consumed there — extending that directory's filter set was out of scope for this phase).

Each result card reuses the Our Professionals directory's own visual card structure (avatar, name, specialty, bio, languages) plus new Online/In-person availability badges, with `BookSessionButton` wired to the real existing booking flow (see below) and a "View profile" link to the therapist's real profile page — no separate/simplified booking UI.

**"Use my current location":** implemented as the spec's own optional fallback path (browser `navigator.geolocation`), but honestly scoped — this app has no reverse-geocoding service anywhere (no mapping API key wired into any route) to turn a raw coordinate into a country name, so rather than fake that translation, the button degrades to a clear message asking for manual entry, which always remains available. Flagging this as a disclosed gap rather than a silent omission.

**Booking integration — reused the existing flow, not a new one:**
- `BookSessionButton` gained an optional `bookingMetadata` prop (`{ sessionType, country, cityOrAddress }`), threaded through to both existing booking branches:
  - **Diary-link branch:** `/api/diary-scheduling` now accepts and stores `path`/`searchSessionType`/`searchCountry`/`searchCityOrAddress` on the `diary_scheduling_events` row it creates.
  - **Native branch:** `IntakeBookingModal` now accepts `bookingMetadata` too. When `sessionType === "in_person"`, the existing "how should we connect?" email/WhatsApp/Zoom picker (none of which is a real in-person option) is replaced with a fixed indicator confirming an in-person session at the therapist's own city/country, and the booking is recorded with the new `in_person` contact channel. `/api/intake-booking` stores the same 3 search fields plus `path`.
- **Session-type-aware availability — disclosed limitation:** the spec asked that "only in-person-compatible slots" show for an in-person search. This app's availability engine (`/api/therapist-availability`, backed by `therapist_weekly_hours`) tracks one shared weekly schedule per therapist with no separate online-hours vs. in-person-hours concept — nothing in the existing schema distinguishes availability by session format. Building that out was a materially larger, separate data-model change than this phase's scope. What this phase *does* guarantee: the booking record is correctly tagged with the session type that was searched, and the in-person booking UI itself no longer offers a remote-only contact channel — but the underlying time-slot list shown is still the therapist's one shared calendar, not filtered by format. Flagging this to Roy explicitly rather than silently claiming full compliance.

**CRM display:** `SessionBookingsTable.tsx`'s existing "Path" column (which already maps `directory`/`ai-support`/etc. to friendly labels) now also maps `browse_therapist` → **"Browse Therapist"** — satisfies the spec's "Source: Browse Therapist" label using the existing column rather than a new one. `diary_scheduling_events` still has no dedicated CRM list view at all (a pre-existing gap, unrelated to this phase — nothing in the admin dashboard reads that table today) — so a Browse Therapist booking that went through a diary-link therapist is tagged correctly in the database but isn't yet visible anywhere in the CRM UI; flagging this as a pre-existing limitation this phase didn't introduce or fix.

**Admin changes:** `TherapistEditForm.tsx` (`/admin` → Our Professionals → edit) gained a "Session types offered" checkbox pair (Online/In-person) and a City field, so admins can actually mark a professional as in-person-capable and give them a city — without this, Browse Therapist's in-person search would never return anyone.

**Wiring chain (all newly threaded, no behavior change to what already worked):** `app/find-your-therapist/page.tsx` now also fetches `getActiveTherapists()` (the same call Our Professionals already uses — no parallel/duplicate therapist list) and passes it through `Hero` → `HeroFindSupportCta` → down to the new `BrowseTherapistModal`. `HeroFindSupportCta` now owns two sibling modal-open booleans (the existing AI/Manual choice modal, and the new Browse Therapist modal) so that clicking "Browse therapist" atomically closes one and opens the other — `ChoiceScreen` gained an `onChooseBrowse` callback for this, kept fully separate from its header "X" (still redirects straight to `/therapists`, unchanged).

**Nested-modal handling:** `BrowseTherapistModal` deliberately uses `z-[85]` — one step *below* the shared booking `Modal.tsx` component's `z-[90]` (used by every step of `BookSessionButton`'s own flows), rather than matching `FindSupportModal`'s `z-[100]`. With this modal below the booking modal's z-index, clicking "Book a session" on a result card lets the real booking flow open naturally on top with no need to hide or unmount anything — no state loss, no visual conflict. **Flagging a related, pre-existing (not introduced by this phase) issue found during this audit:** `FindSupportModal` itself sits at `z-[100]`, *above* the shared booking `Modal.tsx` (`z-[90]`) that `BookSessionButton`'s own sub-modals use — meaning a client using the AI Support wizard's results screen (`StepMatches`) and clicking "Choose therapist" for a non-diary-link therapist would likely have that booking modal render visually behind the AI Support modal's own backdrop. This predates Phase 151 and wasn't in scope to fix here, but is worth a look.

**Accessibility:** `BrowseTherapistModal` implements its own focus trap (Tab/Shift+Tab wrap-around within the dialog, since this modal — like `FindSupportModal` — isn't portalled), moves focus into the dialog on open, restores body-scroll-lock, and closes on Escape. **Disclosed limitation:** Escape may close both this modal and an open booking sub-modal simultaneously if both are open at once, since `BookSessionButton`'s internal modals don't expose a way for a parent to know one is open — not corrected in this phase, since doing so would mean re-architecting `BookSessionButton`'s fully self-contained internal state, which is reused unchanged everywhere else on the site.

**QA:** `npx tsc --noEmit` — one real regression caught and fixed (`tests/unit/TherapistsDirectory.test.tsx`'s hand-built therapist fixture predates this phase and was missing the 3 new required `PublicTherapistRow` fields; added `offers_online`/`offers_in_person`/`city` defaults to it) — confirmed back to the established 16-line baseline (1 pre-existing ESM-import error + 15 across 5 pre-existing test files) with zero new errors afterward. Grepped every new/changed file for the unescaped-apostrophe-in-JSX pattern — every hit is inside a comment or a plain JS string value (state/props), never a literal JSX text node. **Could not get the Jest suite to actually run** — same long-standing, previously-documented sandbox limitation as essentially every phase since 132 (this environment's mounted filesystem makes `npx jest`, even on a single small file, hang past the ~170-180-second command-timeout ceiling with zero output, for both the new test file and pre-existing ones tried as a baseline check). The new test file (`tests/unit/browseTherapistSearch.test.ts`) covers every item the spec's own "Add or update tests for" list named for the parts of this feature that are pure, synchronous logic: required-field validation (all 3 messages), online country-based matching (including the "no country on file = globally available" and case-insensitive rules), in-person country/city matching (including the "missing city data never excludes" rule and the always-null-distance guarantee), and result-count/no-results behavior. Launching the real booking flow and saving `bookingSource` correctly were verified by code inspection (the exact same `BookSessionButton`/`IntakeBookingModal`/diary-scheduling code paths this phase modified, traced end to end) rather than an executed test, for the same sandbox reason. Not verified in a live browser — same standing limitation as every phase since 132.

**Deliverables:**

1. Clicking **Browse therapist** now opens `BrowseTherapistModal` (closing the AI/Manual choice modal first) instead of redirecting straight to `/therapists`.
2. Clients select Online or In-person (with the spec's exact helper text) and enter Country (required for both) plus City/Address (required only for in-person, with autocomplete-via-datalist for country).
3. "Start browsing therapists" shows a live result count and heading ("Therapists available in [Country]" / "Therapists near [City], [Country]"), with filter chips and a "Change search" that preserves the prior selections.
4. Clients select a therapist directly from the filtered results — each result card is the same visual design as the Our Professionals directory.
5. Booking opens the exact same `BookSessionButton`/`IntakeBookingModal`/diary-link flow used everywhere else on the site — no second, simplified booking workflow was built.
6. The completed booking is tagged internally: `session_bookings.path` (native branch) or the new `diary_scheduling_events.path` (diary-link branch) is set to `"browse_therapist"`, with `search_session_type`/`search_country`/`search_city_or_address` also stored on the same row; the CRM's Session bookings table now shows **"Browse Therapist"** as the Path/Source label for these.
7. Polished/responsive/accessible per the notes above, consistent with the existing design system (same card, button, and modal patterns already in use elsewhere) — with the specific, disclosed gaps noted throughout this entry (session-type-aware slot filtering, geolocation-to-country resolution, `/therapists` directory not yet consuming the carried-over query params, `diary_scheduling_events` still has no CRM list view, and the pre-existing `FindSupportModal`/booking-modal z-index issue found during this audit).

**Files changed:** new — `lib/countries.ts`, `lib/browseTherapistSearch.ts`, `components/find-support/BrowseTherapistModal.tsx`, `tests/unit/browseTherapistSearch.test.ts`. Edited — `lib/database.types.ts`, `components/find-support/ChoiceScreen.tsx`, `FindSupportFlow.tsx`, `FindSupportModal.tsx`, `HeroFindSupportCta.tsx`, `components/Hero.tsx`, `app/find-your-therapist/page.tsx`, `components/admin/TherapistEditForm.tsx`, `components/admin/SessionBookingsTable.tsx`, `components/therapists/BookSessionButton.tsx`, `components/intake/IntakeBookingModal.tsx`, `app/api/intake-booking/route.ts`, `app/api/diary-scheduling/route.ts`, `tests/unit/TherapistsDirectory.test.tsx`. Database: 4 migrations applied to production project `iddeoavrlnvwwfopsacy` (listed above).

```
del .git\index.lock
git add -A
git commit -m "Phase 151: Browse Therapist guided search on Find Support page"
git push
```

Roy — same as every phase: please run those four one at a time, and paste back what appears directly after the `git commit` line specifically.

A few things worth your attention before you review, beyond what's already flagged inline above: every existing therapist defaults to online-only until you (or they, if they have dashboard access) go mark specific professionals as in-person-capable and give them a city in the Edit Professional form — until that happens, an in-person Browse Therapist search will come back empty, which is expected given no therapist had this distinction before today. Also worth deciding: do you want the `/therapists` directory itself extended with a real country/session-type filter (right now Browse Therapist's "Browse all professionals" fallback link carries the search over as query params that page doesn't yet read), and do you want the pre-existing AI Support modal/booking-modal z-index issue looked at separately?

---
**Gate:** Per Roy's instruction, each phase stops here for review/approval before the next one starts.

## Phase 152: full pathway therapist listings behind the Home page's three cards

**Request:** Roy's spec referred to these as "About page" cards, but the exact copy quoted ("I've been affected by a crisis" / "I serve, or support someone who serves" / "I'm looking for professional support") is the Home page's three pathway cards (`components/home/Paths.tsx`), not the About/`/find-your-therapist` page — flagging that naming correction up front rather than silently guessing. Ask: clicking each card should show every eligible therapist for that pathway (Resilience/crisis, Veterans, Support/general), not a hard-capped short list; a dynamic heading and live result count; existing filter/sort controls retained; a real profile/booking path per therapist; pagination instead of a fixed cap; and a helpful empty state when nobody qualifies.

**Discovery:** the three cards already linked to `/intake?path=crisis|veteran|general` (unchanged by this phase — `components/home/Paths.tsx` needed no edits at all). The actual restriction lived entirely in `app/intake/page.tsx` and `lib/ai/matchTherapists.ts`: every active therapist was run through the same fuzzy AI/rule-based matcher the Find Your Therapist wizard uses, fed a hardcoded per-path `{ treatmentType, symptoms }` hint, and capped at that file's `MAX_MATCHES = 3` constant — not a `.slice(0,3)`/`.limit(3)` in the intake page itself, but the same practical effect. Results rendered through a bespoke, lightweight component (`IntakeMatchFlow.tsx`) — no photo grid, no specialty/language pills, no profile link, and (unlike the rest of the site's booking flow) always the native intake modal even for diary-link therapists.

More importantly: **no column anywhere recorded which pathway(s) a therapist is actually available for.** The `tracks` (`TrackType[]`) column looked like a candidate but doesn't map to `crisis`/`veteran`/`general` at all (its 4 values are `war_terror`/`antisemitism_diaspora`/`helping_helpers`/`group_support`), and per `matchTherapists.ts`'s own comment, `tracks` is empty on every real therapist row regardless. Building "show every therapist assigned to this pathway, filtered server-side" required adding that data first, not inventing a fuzzy-match substitute.

**Database change:** migration `add_support_pathways_to_therapists` — added `support_pathways text[] not null default '{}'` to `public.therapists` (values are the same 4 keys `/intake?path=` already recognizes: `crisis`, `veteran`, `general`, `helpers`; a therapist can hold more than one). Backfill: every currently-active therapist got `support_pathways = array['general']` — being an active, bookable therapist on this platform already means "available for general professional support," so that pathway isn't empty on day one. `crisis`/`veteran`/`helpers` were deliberately **not** backfilled — those imply specific readiness/training that shouldn't be assumed for an existing record, so those three pathways start empty until an admin explicitly opts a therapist in via the new checkboxes (below). `therapists_public` view recreated (migration `extend_therapists_public_view_for_pathways`) to expose the new column. `TherapistRow`/`PublicTherapistRow` types updated in `lib/database.types.ts`.

**New query — real server/database-driven filtering, not a client-side cap:** `getTherapistsByPathway(pathway)` in `lib/queries.ts` — `.eq("is_verified", true).contains("support_pathways", [pathway])` against `therapists_public` (which already enforces `is_active = true`). This is a scoped exception to `getActiveTherapists()`'s own deliberate stance of not gating on `is_verified` (that decision, and its reasoning, is untouched for the general directory) — Roy's spec explicitly asked this listing to only ever show active *and* verified professionals, so the new function does that, and only that function.

**Admin change:** `TherapistEditForm.tsx` gained an "Intake pathways" checkbox group (Resilience/crisis, Veterans, General support, Helpers) alongside Phase 151's "Session types offered" checkboxes — this is the real, ongoing way an admin assigns a therapist to a pathway going forward.

**Reused the existing directory instead of building a second one:** `app/intake/page.tsx` no longer imports `matchTherapists`/`IntakeMatchFlow` at all. It now renders a dynamic heading (`PATHWAY_HEADING` map: "Available Resilience Support Professionals" / "Available Veterans Support Professionals" / "Available Support Professionals" / a 4th entry for the latent "helpers" path), a live "`N` professional(s) available." count, and the **same `TherapistsDirectory`/`TherapistCard` components the Our Professionals page uses** — full photo, specialty, languages, "View full profile" (via the card's own link to `/therapists/{slug}`), and the real `BookSessionButton` flow (diary-link vs. native, exactly like everywhere else), instead of the old bespoke, native-only `IntakeMatchFlow` card. `TherapistCard` and `TherapistsDirectory` both gained an optional `pathKey` prop (defaulting to `"directory"`, unchanged for every other existing caller) so a booking made from this page is still correctly tagged in `session_bookings.path` — preserving the one useful thing `IntakeMatchFlow`'s direct `pathKey` pass-through used to do.

**Filters/sorting retained, one added:** `TherapistsDirectory`'s existing name/specialty/language/duration/gender filters are unchanged and fully reused here (nothing rebuilt in parallel). Added a new **session format** filter (Online/In-person) using the `offers_online`/`offers_in_person` columns Phase 151 already introduced — the one item on Roy's retained-filters list (language, location, specialty, therapeutic approach, availability, price/fee, session format) that didn't already exist and had real, ready data to back it. Location/price/fee filters were **not** added — no per-therapist numeric fee field or reliable location data exists yet (see Phase 151's own disclosed gaps), and building fake filters against absent data would be worse than not having them; flagging as a known gap rather than a silent omission.

**Pagination:** no pagination pattern existed anywhere in this app before this phase (grepped `app/`/`components/` for "Load more"/pagination/offset/limit — none). Added a plain client-side "Load more" reveal to `TherapistsDirectory` (`PAGE_SIZE = 12`, resets to the first page whenever any filter changes) — the full filtered list is already in memory (one query, no additional network round-trip per page), so this satisfies "don't permanently restrict to 3, but pagination is fine" without needing real server-side pagination for the roster sizes this directory has today. Benefits the general `/therapists` directory too, for free, not just this new pathway page.

**Empty state (zero therapists assigned to a pathway, not zero-after-filtering):** handled directly in `app/intake/page.tsx` before `TherapistsDirectory` even renders — "No therapists were found for this pathway," with three actions: **Change filters** (back to Home to pick a different pathway — there's nothing to "change" yet at this point since no directory filters have rendered), **View all professionals** (`/therapists`), **Contact GESA support** (`/contact`). The separate, pre-existing "filtered down to zero by the sidebar filters" case is still handled by `TherapistsDirectory`'s own built-in no-results message, unchanged.

**Judgment call flagged for Roy:** "Change filters" in the zero-pathway-assigned empty state doesn't literally change a directory filter (none are shown yet) — it links back to the Home page so the visitor can pick a different pathway card instead. Kept the spec's exact label since the intent (give the visitor another way to narrow down, rather than a dead end) is the same; flagging the slight literal mismatch rather than silently reinterpreting it without saying so.

**What was deliberately left untouched:** `components/home/Paths.tsx` (the three cards themselves — no changes needed, they already linked correctly), `components/intake/IntakeMatchFlow.tsx` and `lib/ai/matchTherapists.ts`'s `MAX_MATCHES` (left on disk, unused by this page now, per the no-delete-data convention — `MAX_MATCHES` is still used by the unrelated Find Your Therapist AI Support wizard via `/api/support-match`, so it could not be changed or removed outright even if it weren't also historically un-flagged as a "featured therapists" restriction anywhere in this changelog), and `getRandomMatchedTherapist()` (already unused before this phase, untouched).

**QA:** `npx tsc --noEmit` — two real regressions caught and fixed (`tests/unit/TherapistsDirectory.test.tsx` and `tests/unit/browseTherapistSearch.test.ts`'s hand-built therapist fixtures both predate this phase and were missing the new required `support_pathways` field; added `support_pathways: []` defaults to both). Confirmed back to the established 16-line baseline (1 pre-existing ESM-import error + 15 across 5 pre-existing test files) with zero new errors afterward. Grepped every new/changed file for the unescaped-apostrophe-in-JSX pattern — every hit is inside a comment, and the two new literal JSX apostrophes this phase's own empty-state copy needed were already written with `&apos;`. No new pure-logic unit test was added — this phase's actual new logic is a direct database filter (`getTherapistsByPathway`'s `.eq()`/`.contains()` call), not client-side computation, so there's nothing meaningfully unit-testable the way `lib/browseTherapistSearch.ts` was in Phase 151; the two existing fixture files were updated only to keep type-checking against the new required field. Could not run the Jest suite at all — same long-standing, previously-documented sandbox limitation as every phase since 132 (`npx jest` hangs past the ~170-180s command-timeout ceiling with zero output on this mounted filesystem, confirmed again this phase on both a new and a pre-existing test file). Not verified in a live browser — same standing limitation as every phase since 132.

**Deliverables:**

1. Clicking each of the three Home page cards (Resilience/crisis, Veterans, Support/general) now shows the full, database-filtered therapist list for that pathway via `getTherapistsByPathway()` — not a 3-item AI-matched cap.
2. The list is filtered correctly: `crisis`/`veteran`/`general` map 1:1 to the new `support_pathways` column values, hard-filtered server-side (`.contains()`), with `is_active`/`is_verified` also enforced server-side.
3. More than three therapists display whenever more than three are eligible (no cap in the query, in `TherapistsDirectory`, or in the new "Load more" pagination — the only remaining cap, `MAX_MATCHES = 3`, is no longer reachable from this page at all).
4. A visitor selects any listed therapist via the real `TherapistCard`/`BookSessionButton` flow and proceeds through the exact existing booking/intake process (diary-link or native, same as the Our Professionals page), tagged with the correct pathway in `session_bookings.path`.
5. `/intake?path=crisis` (and `veteran`/`general`) preserve the correct filtered results after a refresh — the pathway comes from the URL's own `path` query param on every request, server-rendered, not client-only state.
6. No inactive or unverified therapist can appear — enforced by the `therapists_public` view (`is_active`) and `getTherapistsByPathway()`'s own `is_verified` filter.

**Files changed:** `lib/database.types.ts`, `lib/queries.ts`, `components/admin/TherapistEditForm.tsx`, `components/TherapistsDirectory.tsx`, `components/TherapistCard.tsx`, `components/intake/IntakeMatchFlow.tsx` (comment only, marking it superseded), `app/intake/page.tsx`, `tests/unit/TherapistsDirectory.test.tsx`, `tests/unit/browseTherapistSearch.test.ts`. Database: 2 migrations applied to production project `iddeoavrlnvwwfopsacy` (`add_support_pathways_to_therapists`, `extend_therapists_public_view_for_pathways`).

```
del .git\index.lock
git add -A
git commit -m "Phase 152: full pathway therapist listings for Home page's three cards"
git push
```

Roy — same as every phase: please run those four one at a time, and paste back what appears directly after the `git commit` line specifically.

One more thing worth your attention: right now, only therapists you (or another admin) explicitly check "Resilience / crisis" or "Veterans" for in their Edit Professional form will show up on those two pathway pages — everyone active was auto-assigned "General support" by the migration, but crisis/veteran start at zero until you go through Our Professionals and check the right boxes for the professionals who actually specialize in those areas. Worth doing that before pointing anyone at those two cards live.
...
## Phase 166: restored the visible gold sheen sweep on the Home hero

**Request:** Roy said the gold hovering/sliding effect on the Home/About hero band was missing, and asked for it back — explicitly clarifying not to bring back the gold background, just make the moving sheen effect visible again.

**Root cause:** the animated sheen sweep itself (`.gold-banner::before`, a diagonal highlight that slides across the band every 7 seconds) was never removed — it's a white highlight, `rgba(255,255,255,0.4)`, which read clearly against the darker `--slate-banner` slate-gray the other three gold-banner pages still use. Once Phase 164 recolored this page's hero to `--clay-soft` (a pale, near-white ivory), that same white sweep became almost invisible against a background nearly as light as the sweep itself — present in the DOM and still animating, just not visible without a live side-by-side diff.

**Fix:** `app/globals.css` gained a `.gold-banner.home-hero::before` override that swaps just the sweep's color to a warm gold (the same `--clay` tone used throughout this page) at higher opacity, so it shows up against the ivory background the same way white used to show up against slate-gray. The background itself (Phase 164/165's ivory) is untouched, per Roy's explicit clarification. Every other `.gold-banner` page keeps the original white sweep exactly as before.

**Verification:** rendered a mockup of the gold sweep at a mid-animation frame against the actual `--clay-soft` background before shipping, rather than assuming from the CSS alone — confirmed it reads as a clear, soft gold shimmer.

**QA:** `npx tsc --noEmit` — confirmed back to the established 16-line baseline, zero new errors. CSS-only change, no JSX touched, so the quote/apostrophe checker wasn't re-run this phase. Not verified in a live browser (animation timing/easing in particular) — same standing sandbox limitation as every phase since 132; worth watching it sweep once it's live.
...
del .git\index.lock
git add -A
git commit -m "Phase 166: restore visible gold sheen sweep on Home hero (ivory background unchanged)"
git push
```

Roy — same as every phase: please run those four one at a time, and paste back what appears directly after the `git commit` line specifically, and confirm the sweep is visible (and still moving smoothly) once it's live.
...
## Phase 176: fixed "Load more" on Our Professionals — root cause was a site-wide hydration failure, not the pagination logic

**Request:** the Our Professionals directory (`/therapists`) showed "Showing 12 of 34 therapists," but clicking "Load more" rendered an empty area instead of the next batch. Roy asked for the actual source of truth/query logic/state handling/rendering logic to be investigated and fixed properly — no hardcoded counts, no visual-only workaround.

**Investigation (in order, each ruling something out with direct evidence before moving on):**
1. Read `components/TherapistsDirectory.tsx`'s pagination in full (`visibleCount`/`PAGE_SIZE = 12`/`filtered.slice(0, visibleCount)`, reset on filter change, "Showing X of Y" line) — structurally correct, no bug found.
2. Read `app/therapists/page.tsx` and `lib/queries.ts`'s `getActiveTherapists()` — a single, unfiltered `therapists_public` view query with no `.limit()`/`.range()`, so the count and the paginated list already share one eligibility source. Confirmed the view definition directly against the live Production database (`pg_get_viewdef`): `is_active = true`, nothing else — matches what the code assumes.
3. Queried live Production data directly (34 active therapists, 145 total): zero null `languages`/`specialties`/`session_lengths`/`full_name`/`slug`, and zero duplicate `id`s — ruled out a bad-record/duplicate-key hypothesis with real data, not assumption.
4. Reproduced the bug directly against `https://gesa-platform.vercel.app/therapists` in a live browser session (no local dev server exists in this sandbox). Clicking the real "Load more" button produced **zero DOM change** — count text, card count, and HTML all identical before and after. This ruled out the pagination logic itself: the click handler wasn't computing the wrong thing, it was doing nothing at all.
5. The browser console showed a repeating cluster of production React errors on every page load, on every route (not just `/therapists` — confirmed on `/login` too, which shares nothing with the directory page except the global root layout): minified errors **#329, #418, #423, #425**. Decoded against react.dev: #425 = "Text content does not match server-rendered HTML," #418 = "Hydration failed," #423 = "React was able to recover by instead client rendering the entire root," #329 = "Unknown root exit status" (an internal React invariant, meaning the recovery attempt itself failed).
6. Ruled out two `localStorage`-driven candidates (the `gesa-lang` RTL-sync script, the accessibility widget's saved settings) by direct inspection and by clearing `localStorage` entirely and reloading — the identical error cluster still fired.
7. Directly confirmed the consequence: scrolled a therapist card into view and re-checked its computed style — `opacity: 0` forever, even after `scrollIntoView`. The card grid's scroll-triggered entrance animation (`StaggerGroup`/`StaggerItem`, used by every card and by the global Footer on every page) never fires. This, plus the inert click handler, means **no React effect was running at all** on this route — consistent with "recovery failed, root left dead," not with a logic bug anywhere in the directory component.

**Root cause found:** `framer-motion`'s own `useReducedMotion()` hook reads `window.matchMedia("(prefers-reduced-motion)")` synchronously *during render* — not inside an effect. On the server (no `window`) it always returns `null`; on the client's very first render — the one React's hydration pass compares byte-for-byte against the server's HTML — it can already return a real `true`/`false`. Six motion primitives in `components/motion/` (`Reveal`, `StaggerGroup`, `StaggerItem`, `ParallaxLayer`, `NewsTicker`, `HorizontalScroll`, `ScrollText`, `ParallaxMedia`) fed that raw value straight into the variant/branch used for the very first paint. For any visitor (or the automated browser used to reproduce this) whose OS/browser reports `prefers-reduced-motion: reduce`, the server rendered the full-motion "hidden" style (confirmed directly in the SSR HTML: `opacity:0; transform: translateY(52px) scale(0.97)` on every card) while the client's first hydration pass rendered the reduced-motion style (`opacity:0` only, no transform) for the same nodes — a genuine, guaranteed attribute mismatch. The global `Footer` alone renders roughly a dozen of these nodes on *every* route, so a reduced-motion visitor hits a dozen simultaneous mismatches on first load. React logged the hydration errors, tried to recover by fully client-rendering the root, and — likely because of how many nodes disagreed at once, combined with the `Suspense` boundary already in the tree (`GlobalContentGate`, used for the admin Page Editor's live-preview overlay) — that recovery attempt itself hit an internal invariant (#329) and never completed, leaving the page's entire React root inert: no effects ever ran, so nothing on the page was interactive, "Load more" included. This was never a bug in the pagination code, the query, or the data — all three were already correct.

**Fix — `components/motion/useSafeReducedMotion.ts` (new):** a wrapper hook that always reports `false` (matching the server's `Boolean(null)`) until after the component has mounted, then switches to the real preference once an effect has run — by which point hydration has already succeeded and there's nothing left to mismatch. The only cost is that a reduced-motion visitor's very first paint briefly uses the full-motion "hidden" variant before switching to the reduced one; since nothing has animated into view yet at that point, this is imperceptible. Swapped every direct `useReducedMotion` import in `components/motion/Reveal.tsx`, `StaggerReveal.tsx` (both `StaggerGroup` and `StaggerItem`), `ParallaxLayer.tsx`, `NewsTicker.tsx`, `HorizontalScroll.tsx`, `ScrollText.tsx`, and `ParallaxMedia.tsx` for this hook. Left `components/ui/Modal.tsx` and `components/motion/AnimatedCounter.tsx` untouched — both already only use the raw hook safely (Modal never renders anything until a post-mount `mounted` flag is true; AnimatedCounter only reads it inside a `useEffect`, never in its rendered output), so neither was part of the bug.

**Also fixed while in this code, per the spec's other requirements:**
- **`lib/queries.ts`'s `getActiveTherapists()`** — added `.order("id")` as a tie-breaker after `.order("full_name")`. Two therapists sharing a name had no guaranteed stable relative order before; `id` is unique, so the combined sort is now fully deterministic regardless of how many rows come back in a given request.
- **`components/TherapistsDirectory.tsx`** — "Load more" now also announces a one-shot, visually hidden message ("12 more therapists loaded. Showing 24 of 34.") via a dedicated `aria-live="polite"` status region, separate from the existing persistent "Showing X of Y" line (which was already `aria-live` and already correct). The button's accessible name now states exactly how many more it will load (`aria-label="Load N more therapists"`), and the announcement clears whenever a filter changes pagination back to page one.
- **Loading/error states:** the spec asked for a loading state and an error-with-retry state for "Load more." This directory's pagination is genuinely synchronous, in-memory slicing of data already fetched once on the server (`therapists` prop) — there is no network request per click for a spinner to represent or for a retry to re-attempt, and fabricating a fake delay or a fake failure mode would be dishonest UI, not a fix. What looked like "Load more silently failing" was never a network failure — it was this phase's hydration bug making the button's click handler never run at all; with that fixed, the click is synchronous and always succeeds.

**Tests:** added a `describe("Load more", ...)` block to `tests/unit/TherapistsDirectory.test.tsx` (34 fixture therapists, matching the real production count) covering: initial 12-of-34 render, clicking through to 24-of-34 with no duplicate cards, the accessible announcement text, the button disappearing once all 34 are shown, and pagination/announcement resetting when a filter changes. Added `tests/unit/useSafeReducedMotion.test.tsx`, a direct regression test for the actual root cause: asserts the hook reports `false` on the very first render even when `matchMedia` reports `prefers-reduced-motion: reduce`, and only switches to the real value after mount.

**Verification:** reproduced the original bug live against production (browser automation) before touching any code, and confirmed via direct DOM inspection (computed `opacity`, a scripted click on the real button node) exactly why it was inert, before forming the fix. **`npx tsc --noEmit` and `npx jest` could not be run in this sandbox** — the shell environment has been completely wedged since partway through Phase 175 and never recovered (confirmed unusable again this phase). Every change was instead re-read in full by hand after editing, and the new/changed tests were written to fail clearly against the pre-fix code and pass against the post-fix code by inspection. Please run both commands yourself before deploying — this is the same caveat as Phase 175, and matters more here since it touches shared motion primitives used on every page site-wide, not just the therapists directory.

**Files changed:** `lib/queries.ts`, `components/TherapistsDirectory.tsx`, `components/motion/Reveal.tsx`, `components/motion/StaggerReveal.tsx`, `components/motion/ParallaxLayer.tsx`, `components/motion/NewsTicker.tsx`, `components/motion/HorizontalScroll.tsx`, `components/motion/ScrollText.tsx`, `components/motion/ParallaxMedia.tsx`, `tests/unit/TherapistsDirectory.test.tsx`. **New:** `components/motion/useSafeReducedMotion.ts`, `tests/unit/useSafeReducedMotion.test.tsx`.

```
del .git\index.lock
git add -A
git commit -m "Phase 176: fix site-wide hydration failure breaking Load More and other interactivity"
git push
```

Roy — please run `npx tsc --noEmit` and `npx jest` locally before these four, same as last phase (the sandbox shell is still wedged). This one is worth testing by hand too: open `/therapists` on the live site after deploying and click "Load more" a couple of times, and if you have "reduce motion" turned on anywhere in your OS/browser accessibility settings, it's worth testing with that on as well, since that's the exact condition this phase fixes.
