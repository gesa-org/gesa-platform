...
// Phase 145 — Roy asked to remove the old standalone Find Support page (the
// "A guided match, just for you" AI/Manual Support choice screen) and put
// the existing About page's content here instead: this file is now, byte
// for byte in structure, what used to live at app/about/page.tsx (see that
// file's own comment — it's left in place but unreachable, since `/about`
// now permanently redirects here via next.config.mjs). Content Manager
// still edits this page through the same two site_content keys/pageKey
// ("about" — page_about_hero + page_about_sections) it always did; only the
// URL, nav label, and browser-tab title changed, not the Content Manager
...
// margin. Phase 145 — SiteFooterSlot's REVEAL_ROUTES now lists
// "/find-your-therapist" instead of "/about" to match this move.
//
  ...
  const page = (
    <div className="reveal-page__main">
      ...
      {/* Phase 145 — `id="how-it-works"` added so the Hero's primary CTA
          (now an in-page anchor, since this Hero renders on the same page
